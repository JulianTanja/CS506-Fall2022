...
def test_euclidean():
    ...
    assert sim.euclidean_dist(
        [0, 0, 0], [1, 0, 0]) == sim.euclidean_dist([1, 0, 0], [0, 0, 0])
    ...
    assert sim.euclidean_dist(x, y) <= sim.euclidean_dist(
        x, z) + sim.euclidean_dist(z, y)
...
def test_manhattan():
    ...
    # triangle inequality
    z = _generate_rand_point(dimension)
    assert sim.manhattan_dist(x, y) <= sim.manhattan_dist(
        x, z) + sim.manhattan_dist(z, y)
...
def test_jaccard():
    ...
    # triangle inequality
    z = _generate_rand_point(dimension)
    assert sim.jaccard_dist(x, y) <= sim.jaccard_dist(
        x, z) + sim.jaccard_dist(z, y)
